  ...
  <div>
    ...
    <div class="panel-body">
      ...
      <div class="col-sm-12" style="margin-top: 10px;">
        <el-button type="primary" class="pull-right"  @click="createArticle()" icon="el-icon-edit-outline"> Modifier l'Article</el-button>
      </div>
    </div>
  ...
  methods: {
    ...
    getArticle: function() {
        ...
        this.$http.get('http://www.skear.fr/server/web/api/articles', {
          ...
          .then(function (response) {
              self.pathcoverimage = response.data["hydra:member"][0].pathcoverimage;
              self.title = response.data["hydra:member"][0].title;
              self.categorychoose = response.data["hydra:member"][0].categoryarticle["@id"];
              self.content = response.data["hydra:member"][0].content;
              self.user.pathavatar = response.data["hydra:member"][0].user.pathavatar;
              self.user.username = response.data["hydra:member"][0].user.username;
              self.user.useridpost = response.data["hydra:member"][0].user["@id"];
              self.articleid = response.data["hydra:member"][0].id;
              self.finishLoader();
          })
          .catch(function (error) {
              alert(error);
    ...
    createArticle(){
      ...
      if(this.title == ""){
        ...
        this.$confirm('Voulez vous créer cette article ?', 'Warning', {
          ...
          this.startLoader();
          // content
          var contentsend = {"title": this.title, "content": this.content , "pathcoverimage": this.pathcoverimage, "published": true, "user": this.user.useridpost , "categoryarticle": this.categorychoose}
          // post
          this.$http.put('http://www.skear.fr/server/web/api/articles/'+this.articleid, contentsend, {
              headers: {'Authorization': 'Bearer ' + window.localStorage.token}
            })
            .then(response => {
              this.$notify({
